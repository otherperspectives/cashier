...
class ApiClient
{
     ...
     * @return object Response of fastspring
     */
    public function getEvents($type, $parameters = [])
    {
        return $this->apiRequest('GET', implode('/', ['events', $type]), $parameters, [], []);
    }
     ...
     * @param array $payload
     *
     * @see https://docs.fastspring.com/integrating-with-fastspring/fastspring-api/events
     *
     * @return object Response of fastspring
     */
    public function updateEvent($eventId, $payload = [])
    {
        return $this->apiRequest('POST', implode('/', ['events', $eventId]), [], [], $payload);
    }
}
